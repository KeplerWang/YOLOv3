...
def get_map(MINOVERLAP, draw_plot, path='./map_out'):
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            if len(prec) > 0:
                print("%-30s" % text, "\t||\tscore_threhold=0.5 : " + "F1=" + "{0:.2f}".format(F1[score05_idx]) \
                      + " ; Recall=" + "{0:.2f}%".format(rec[score05_idx] * 100) + " ; Precision=" + "{0:.2f}%".format(
                    prec[score05_idx] * 100))
            else:
                print("%-30s" % text, "\t||\tscore_threhold=0.5 : F1=0.00 ; Recall=0.00% ; Precision=0.00%")
    ...
    if draw_plot:
        window_title = "mAP"
        plot_title = "mAP = {0:.4f}%".format(mAP)
